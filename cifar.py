...
def load_test_val_from_folder(data_dir='data_original', batch_size=32):
    ...
    return valloader, testloader, classes

def load_dataset_kfold(data_dir='../data_original/train'):
    transform = transforms.Compose([
        transforms.Resize((224, 224)),
        transforms.ToTensor()
    ])
...
# models = resnet18, mobilenetv2, resnet18_mobilenetv3, vgg16_mobilenetv3
...
batch_size = 16
print_dataset_sizes('../data_original')

#dataset = load_cifar10(batch_size)
dataset = load_dataset_kfold()
...
for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
    ...
    _, testloader, classes = load_test_val_from_folder(batch_size=batch_size)
    ...
    _, test_acc, test_prec, test_rec, test_f1 = trainer.evaluate(testloader, type='teste')
    print(f"Acurácia no Teste: {test_acc:.4f} | Precision: {test_prec:.4f} | Recall: {test_rec:.4f} | F1-Score: {test_f1:.4f}")
...
print(f"Todas as métricas salvas em {output_path}")
