...
import random
from albumentations import HorizontalFlip, VerticalFlip, CLAHE, Compose, ColorJitter, GaussianBlur
from glob import glob
import cv2
import os
from tqdm import tqdm

def balance_dataset_to_common_target(train_dir):
    ...
    # Valor alvo comum = classe com mais imagens
    max_count = max(class_counts.values())
    print(f"\nClasse majoritária tem {max_count} imagens. Todas as demais serão aumentadas até esse valor.")

    for class_path, current_count in class_counts.items():
    ...
    print("\nAumento finalizado com sucesso!")
...
balance_dataset_to_common_target(train_dir='../data_ceratite_alternativo_augment/train')